...
const feature = loadFeature('./features/subjects.feature');
...
defineFeature(feature, (test) => {
    ...
    test('Create, filter, edit and delete a subject', ({ given, when, then }) => {
        ...
        when('I open the subjects list', async () => {
            const headerButtons = await page.$$('header button');
            if (headerButtons.length > 0) await headerButtons[0].click();
            const navXpath = "//li//span[contains(., 'Materias')]";
        ...
        when('I attempt to create a subject with invalid data', async () => {
        ...
        when('I create a valid new subject', async () => {
            const [nameInput] = await page.$x("//label[contains(., 'Nombre')]/following::input[1]");
            await nameInput.click({ clickCount: 3 });
            await nameInput.type('Seguridad');
            const [codeInput] = await page.$x("//label[contains(., 'Código')]/following::input[1]");
            ...
            expect(alertText).toContain('Materia creada correctamente');
        });

        then('I should see the new subject in the list', async () => {
            const headerButtons = await page.$$('header button');
            if (headerButtons.length > 0) await headerButtons[0].click();
            const navXpath = "//li//span[contains(., 'Materias')]";
            await page.waitForXPath(navXpath);
            ...
            const rows = await page.$x("//tr[.//text()[contains(., 'Seguridad')]]");
            expect(rows.length).toBeGreaterThan(0);
        });

        when('I apply correct filters for that subject', async () => {
            const [nameFilter] = await page.$x("//label[contains(., 'Nombre')]/following::input[1]");
            await nameFilter.type('Seg');

            await page.waitForTimeout(500);
        });

        then('the filters work and I see that subject', async () => {
            const rows = await page.$x("//tr[.//text()[contains(., 'Seguridad')]]");
            expect(rows.length).toBeGreaterThan(0);
        });
        ...
        when('I edit the subject', async () => {
            const [row] = await page.$x("//tr[.//text()[contains(., 'Seguridad')]]");
            ...
            await page.waitForSelector('div[role="alert"]');
            const alertText = await page.$eval('div[role="alert"]', (n) => n.textContent || '');
            expect(alertText).toContain('Materia actualizada correctamente');
        });
        ...
        then('I should see the changes in the list', async () => {
            ...
            const rows = await page.$x("//tr[.//text()[contains(., 'Seguridad Editado')]]");
            expect(rows.length).toBeGreaterThan(0);
        });

        when('I delete the subject', async () => {
            const [row] = await page.$x("//tr[.//text()[contains(., 'Seguridad Editado')]]");
            const [editBtn] = await row.$x(".//*[contains(@data-testid, 'edit-button-')]");
            await editBtn.click();
            ...
            await page.waitForSelector('div[role="alert"]');
            const alertText = await page.$eval('div[role="alert"]', (n) => n.textContent || '');
            expect(alertText).toContain('Materia eliminada correctamente');
        });
