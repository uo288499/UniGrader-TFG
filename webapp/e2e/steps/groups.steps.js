...
defineFeature(feature, (test) => {
    beforeAll(async () => {
        ...
        if (headerButtonsAfterStud.length > 0) await headerButtonsAfterStud[0].click();
        await page.waitForXPath(usersNavXpath);
        ...
        const [editProBtn] = await proRow[0].$x(".//*[contains(@data-testid, 'edit-button-')]");
        await editProBtn.click();
        await page.waitForSelector('[data-testid="user-form-page"]');
        const [uniProInput] = await page.$x("//label[contains(., 'Universidad')]/following::input[1]");
        await uniProInput.click();
        const [uniProOption] = await page.$x("//li[contains(., 'Test University')]");
    ...
    test('Create, filter, edit and delete a group', ({ given, when, then }) => {
        ...
        when('I open the groups list', async () => {
            const headerButtons = await page.$$('header button');
            if (headerButtons.length > 0) await headerButtons[0].click();
            const navXpath = "//li//span[contains(., 'Grupos')]";
        ...
        when('I attempt to create a group with invalid data', async () => {
            const newBtn = await page.$x("//button[contains(., 'Nuevo Grupo')]");
            await newBtn[0].click();
        ...
        then('I should see validation errors', async () => {
            const errStud = await page.$x("//p[contains(., 'Debes asignar al menos un estudiante al grupo')]");
            expect(errStud.length).toBeGreaterThan(0);
        });

        when('I create a valid new group', async () => {
            const availableStud = await page.$x("//li[contains(., 'student@test.com')]");
            ...
            expect(alertText).toContain('Grupo creado correctamente');
        });

        then('I should see the new group in the list', async () => {
            const headerButtons = await page.$$('header button');
            if (headerButtons.length > 0) await headerButtons[0].click();
            const navXpath = "//li//span[contains(., 'Grupos')]";
            await page.waitForXPath(navXpath);
        ...
        when('I apply correct filters for that group', async () => {
            const [nameFilter] = await page.$x("//label[contains(., 'Nombre')]/following::input[1]");
            await nameFilter.type('A');

            await page.waitForTimeout(500);
        });

        then('the filters work and I see that group', async () => {
            const rows = await page.$x("//tr[.//text()[contains(., 'Cyberseguridad')]]");
            expect(rows.length).toBeGreaterThan(0);
        });
        ...
        when('I edit the group', async () => {
            ...
            await page.waitForSelector('div[role="alert"]');
            const alertText = await page.$eval('div[role="alert"]', (n) => n.textContent || '');
            expect(alertText).toContain('Grupo actualizado correctamente');
        });
        ...
        then('I should see the changes in the list', async () => {
            ...
            const rows = await page.$x("//tr[.//text()[contains(., 'A Editado')]]");
            expect(rows.length).toBeGreaterThan(0);
        });

        when('I delete the group', async () => {
            const [row] = await page.$x("//tr[.//text()[contains(., 'A Editado')]]");
            const [editBtn] = await row.$x(".//*[contains(@data-testid, 'edit-button-')]");
            await editBtn.click();
            ...
            await page.waitForSelector('div[role="alert"]');
            const alertText = await page.$eval('div[role="alert"]', (n) => n.textContent || '');
            expect(alertText).toContain('Grupo eliminado correctamente');
        });
