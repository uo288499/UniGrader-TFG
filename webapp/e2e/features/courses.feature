Feature: Manage courses

  Scenario: Create, filter, edit and delete a course
    Given I am logged in as the admin of a university
    When I open the courses list
    Then the list should be empty

    When I attempt to create a course with invalid data
    Then I should see validation errors

    When I create a valid new course
    Then I should see the new course in the list

    When I apply correct filters for that course
    Then the filters work and I see that course

    When I apply non matching filters
    Then the filters work and the no-results state appears

    When I edit the course
    Then I should see the changes in the list

    When I delete the course
    Then the list should be empty again